...
| `/workspace` | `src/app/workspace/page.tsx` | WorkspaceEntry | 工作台入口（自动跳转最新会话） |
| `/workspace/home` | `src/app/workspace/home/page.tsx` | WorkspaceEntry (no redirect) | 工作台首页（不自动跳转） |
| `/workspace/new` | `src/app/workspace/new/page.tsx` | WorkspaceEntry (no redirect) | 新建会话页（不自动跳转） |
...
### workspace/
- **workspace-entry.tsx** -- 工作台入口页。加载会话列表，支持 `autoRedirectToLatest` prop 控制是否自动跳转最新会话；无会话则显示创建表单。含 useSchemaGate 前置检测。
...
| `workspace-explicit-entry-pages.test.tsx` | 工作台显式入口页（/workspace/home, /workspace/new） |
...
| `prd-store-helpers.test.ts` | PRD store helpers 单元测试 |
...
A: `/workspace` 会自动跳转到最新会话（如果有），`/workspace/home` 和 `/workspace/new` 不会自动跳转，用于显式创建新会话或查看会话列表。
...
apps/web/
  ...
  src/
    ...
    test/
      ...
      workspace-explicit-entry-pages.test.tsx
...
| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-04-12 | UPDATED | 新增工作台显式入口路由（/workspace/home, /workspace/new）；WorkspaceEntry 支持 autoRedirectToLatest prop；新增测试文件：workspace-explicit-entry-pages.test.tsx, prd-store-helpers.test.ts |
